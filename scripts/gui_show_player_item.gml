...
for (i = 0; i < ds_list_size(plyr_target.plyr_items); i++) {
    item = ds_list_find_value(plyr_target.plyr_items,i)
    text = item[? "name"]
    str_height = string_height("ATK")
    
    draw_sprite(item_icons[item[? "type"]],0,panel_start_x+10,panel_start_y+50 + (i*str_height)+ 2)
    draw_text(panel_start_x+36,panel_start_y+50 + (i*str_height),text)
    if (item[? "dmg"] > 0) {
        draw_text(panel_start_x+panel_width/2,panel_start_y+50 + (i*str_height)+2,"ATK: +" + string(item[? "dmg"]))
    }
    else {
         draw_text(panel_start_x+panel_width/2,panel_start_y+50 + (i*str_height)+2,"ATK: " + string(item[? "dmg"]))
    }
